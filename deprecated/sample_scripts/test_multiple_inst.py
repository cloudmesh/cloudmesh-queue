...
logging.basicConfig(filename='{out_dir}/test_python_script_{instance}_{dt}.log'.format(out_dir=out_dir, instance=instance, dt=datetime.now().strftime("%m%d%Y_%H%M%S")),
                    level=logging.DEBUG,
                    format='%(asctime)s|%(filename)s|%(levelname)s|%(message)s')

logging.info("Started the instance {inst} of python script at {dt}".format(inst=instance, dt=datetime.now()))
