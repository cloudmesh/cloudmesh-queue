...
# cms set user=$USER
#
# pytest -v --capture=no tests/test_06_scheduler_FIFO_with_crashes.py
# pytest -v  tests/test_06_scheduler_FIFO_with_crashes.py
# pytest -v --capture=no  tests/test_06_scheduler_FIFO_with_crashes.py::TestSSHJob::<METHODNAME>
###############################################################
import getpass
...
class TestQueue:
    ...
    def test_queue_run(self):
        ...
        for i in range(15):
            if i !=4 and i!=5 and i!=9 and i!=10 and i!=11 and i!=12 and i!=13:
                assert f'job{i}' in completed_jobs
            else:
                # job 4 was deleted from queue
                # job 5 was skipped bc undefined
